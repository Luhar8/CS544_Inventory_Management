...
class AsyncQuicServer(QuicConnectionProtocol):
    def __init__(self, *args, **kwargs):
        ...
        self._handlers: Dict[int, InventoryServerRequestHandler] = {}
        self._client_handler: Optional[InventoryClientRequestHandler] = None
    ...
    def _attach_client_handler(self): 
        if self._mode == CLIENT_MODE:
            self._client_handler = InventoryClientRequestHandler(
    ...
    def _quic_server_event_dispatch(self, event):
        ...
        if isinstance(event, StreamDataReceived):
            if event.stream_id not in self._handlers:
                handler = InventoryServerRequestHandler(
                        authority=self._quic.configuration.server_name,
                        connection=self._quic,
                        protocol=self,
                        scope={},
                ...
                asyncio.ensure_future(handler.launch_inventory())
            else:
...
async def run_client(server, server_port, configuration):
    try:
        async with connect(server, server_port, configuration=configuration, create_protocol=AsyncQuicServer) as client:
            ...
            # Proceed with any client-specific protocols
            await asyncio.ensure_future(client._client_handler.launch_inventory())
    except Exception as e:
        print(f"Failed to connect or process data: {e}")


class InventoryServerRequestHandler:
    def __init__(
        self,
        *,
        authority: bytes,
    ...
    async def launch_inventory(self):
        qc = EchoQuicConnection(self.send, 
                self.receive, self.close, None)
        await echo_server.inventory_server_proto(self.scope, 
            qc)
        
        
class InventoryClientRequestHandler(InventoryServerRequestHandler):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
    ...
    async def launch_inventory(self):
        ...
        await echo_client.inventory_client_proto(self.scope, 
            qc)
